...
coeffs_path = os.path.join(BASE_DIR, "data", "trajectories", "verification_coeffs.npy")
dataset_out_path = os.path.join(BASE_DIR, "data", "training_datasets", "verification_dataset.npz")                  
...
def main():
    if not os.path.exists(coeffs_path):
        raise FileNotFoundError(f"Cannot find verification coefficients at {coeffs_path}")
    ...
    print("Precomputing verification trajectory...")
    ...
    print("Launching MuJoCo Viewer for Verification Data...")
    ...
    print(f"Verification simulation complete! Clean data saved to: {dataset_out_path}")
